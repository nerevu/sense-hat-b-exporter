...
class TCS34725:
    ...
    def __init__(self, address=TCS34725_I2C_ADDRESS, debug=False):
        if smbus is None:
            return
    ...
    def GetRGB565(self):
        ...
        self.RG565 = (
            ((RGB565_R >> 3) << 11) | ((RGB565_G >> 2) << 5) | (RGB565_B >> 3)
        ) & 0xFFFF
    ...
    def lux(self):
        ...
        lux = (
            TCS34725_R_Coef * (float)(r_comp)
            + TCS34725_G_Coef * (float)(g_comp)
            + TCS34725_B_Coef * (float)(b_comp)
        ) / cpl
        return lux
